...
Type:
	...
	;
ListVar:
	 X {char* scope = return_string(current_scope); if(search(ht,num_extract(scope),scope,$1.string)){printf("Declaring twice!\n");} else {insert(ht, num_extract(scope), scope, $1.string,$0.string,"(null)");}}
	|Assign {char* scope = return_string(current_scope); if(search(ht,num_extract(scope),scope,$1.string)){printf("Declaring twice!\n");} else {insert(ht, num_extract(scope), scope, $1.string,$1.type,$1.val);}}
  |ListVar ',' Assign { char* scope = return_string(current_scope); if(search(ht,num_extract(scope),scope,$3.string)){printf("Declaring twice!\n");} else {insert(ht, num_extract(scope), scope, $3.string,$0.string,$3.val);}}
  |ListVar ',' X { char* scope = return_string(current_scope); if(search(ht,num_extract(scope),scope,$3.string)){printf("Declaring twice!\n");} else {insert(ht, num_extract(scope), scope, $3.string,$0.string,"(null)");}}
	|T_DIMS print_id {char* scope = return_string(current_scope); if(search(ht,num_extract(scope),scope,$2.string)){printf("Declaring twice!\n");} else {insert(ht, num_extract(scope), scope, $2.string,$0.string,"(null)");}}
	|T_DIMS T_DIMS print_id {char* scope = return_string(current_scope); if(search(ht,num_extract(scope),scope,$3.string)){printf("Declaring twice!\n");} else {insert(ht, num_extract(scope), scope, $3.string,$0.string,"(null)");}}
	|print_id T_DIMS '=' T_NEW Type T_DIMS {char* scope = return_string(current_scope); if(search(ht,num_extract(scope),scope,$1.string)){printf("Declaring twice!\n");} else {if($6.number==-1){yyerror("error");}insert(ht, num_extract(scope), scope, $1.string,$0.string,"(null)");}}
	|T_DIMS print_id '=' T_NEW Type T_DIMS {char* scope = return_string(current_scope); if(search(ht,num_extract(scope),scope,$2.string)){printf("Declaring twice!\n");} else {if($6.number==-1){yyerror("error");}insert(ht, num_extract(scope), scope, $2.string,$0.string,"(null)");}}
	;
X:
	 print_id
	//|Assign
...
{ 
 ...
 if(search(ht,num_extract(scope),scope,$1.string))
 {
	...
	fprintf(stderr,"types of things:%s %s",$3.type,node->type);
 ...
 {
	fprintf(stderr,"types of things:%s %s",$0.string,$3.type);
...
void display(struct hash* ht)
 {
   ...
   fprintf(t,"NAME\tTYPE\tSCOPE\tVALUE\n");
